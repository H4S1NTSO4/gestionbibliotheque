...
import com.bibliothecaire.gestionbibliotheque.service.LivreImportService;
...
public class LivreController {
  ...
  @FXML
  private TableColumn<Livre, String> colReference;
  @FXML
  ...
  @FXML
  private TableColumn<Livre, Integer> colDisponibles;
  @FXML
  private TableColumn<Livre, String> colEtat;
  @FXML
  private TableColumn<Livre, Void> colActions;
  // Champs FXML de la barre de recherche et filtres
  @FXML
  private TextField txtRecherche;
  @FXML
  private ComboBox<String> cmbCategorieFilter;
  ...
  // Repositories, Services et données
  private LivreRepository livreRepository;
  private CategorieLivreRepository categorieRepository;
  private LivreImportService importService; // NOUVEAU: Service d'importation
  ...
  public void initialize() {
    ...
    importService = new LivreImportService(); // Initialisation du service
  ...
  /**
   * NOUVEAU: Gère l'action d'importer un fichier Excel de livres.
   * Utilise le LivreImportService pour la logique métier.
   */
  @FXML
  private void handleImporterLivres() {
    FileChooser fileChooser = new FileChooser();
    fileChooser.setTitle("Sélectionner le Fichier d'Importation");
    fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("Fichiers Excel", "*.xlsx"));
    File selectedFile = fileChooser.showOpenDialog(null);

    if (selectedFile != null) {
      // 1. Validation du format des en-têtes
      if (!ExcelImporter.validerFormatLivreExcel(selectedFile)) {
        showError("Erreur de Format", "Le fichier Excel ne correspond pas au format attendu (Ref, Titre, Auteur, Nb Exemplaire, Catégorie).");
        return;
      }

      // 2. Exécution de l'importation dans un thread séparé (Task)
      Task<Integer> importTask = new Task<>() {
        @Override
        protected Integer call() throws Exception {
          return importService.importerEtSauvegarderLivres(selectedFile);
        }
      };

      importTask.setOnSucceeded(e -> {
        int count = importTask.getValue();
        showInfo("Importation Réussie", count + " livres ont été insérés dans la base de données.");
        handleRefresh(); // Recharger les données et réinitialiser les filtres
      });

      importTask.setOnFailed(e -> {
        hideLoading();
        Throwable exception = importTask.getException();
        showError("Erreur d'Importation", "Un problème est survenu lors de l'importation. Détail : " + exception.getMessage());
        exception.printStackTrace();
      });

      showLoading(); // Afficher l'overlay pendant le traitement
