...
public class CategorieLivreDAO implements CategorieLivreRepository {
  ...
  // ==========================================================
  // NOUVELLE MÉTHODE REQUISE POUR L'IMPORTATION EXCEL (RÈGLE 3)
  // ==========================================================
  @Override
  public CategorieLivre findByGenre(String genre) {
    String sql = "SELECT id_categorie, genre FROM categorie_livre WHERE genre = ?";
    ...
    try (Connection conn = getConnection();
      ...
      try (ResultSet rs = stmt.executeQuery()) {
        if (rs.next()) {
          categorie = mapResultSetToCategorieLivre(rs);
        }
      }
    } catch (SQLException e) {
      System.err.println("Erreur lors de la recherche de la catégorie par genre: " + genre);
      e.printStackTrace();
    }
    return categorie;
  }
